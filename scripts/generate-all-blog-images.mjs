...
for (const article of all60Articles) {
  ...
  // Format title for wrapping if needed
  const rawTitle = article.pt.title.replace(/&/g, '&amp;');
  ...
  <g transform="translate(50, 260)">
    ...
    <text x="12" y="17" fill="${style.accent}" font-family="system-ui, -apple-system, sans-serif" font-size="11" font-weight="700" letter-spacing="1">${style.badge}</text>
    
    <!-- Title Line 1 -->
    <text x="0" y="65" fill="#FFFFFF" font-family="system-ui, -apple-system, sans-serif" font-size="25" font-weight="800">${line1}</text>
    ${line2 ? `<text x="0" y="98" fill="#E2E8F0" font-family="system-ui, -apple-system, sans-serif" font-size="23" font-weight="700">${line2}</text>` : ''}
    
    <!-- Footer Brand & Date -->
    <text x="0" y="${line2 ? '135' : '110'}" fill="#94A3B8" font-family="system-ui, -apple-system, sans-serif" font-size="13" font-weight="500">CONEXUS • Guia Estratégico Digital • 2026</text>
  </g>

  <!-- Top Brand Watermark -->
  <text x="790" y="50" fill="#FFFFFF" opacity="0.3" font-family="system-ui, -apple-system, sans-serif" font-size="16" font-weight="800" text-anchor="end" letter-spacing="2">CONEXUS</text>
</svg>`;

  // Save as WebP / SVG fallback compatible asset
  const targetSvg = path.join(imagesDir, imageName.replace('.webp', '.svg'));
  const targetWebp = path.join(imagesDir, imageName);

  fs.writeFileSync(targetSvg, svgContent, 'utf-8');
  // Also write the file with .webp name so browsers loading /assets/images/[name].webp directly find it perfectly
  fs.writeFileSync(targetWebp, svgContent, 'utf-8');
}

console.log(`Generated ${all60Articles.length} SVG/WebP banner assets in ${imagesDir}.`);
